...
int main(int argc, char** argv) 
{
  ...
  string displacement_fn = "data/displacements_";
  string pred_fn = "data/pred_";
  string residuals_fn = "data/residuals_";
  string minmax_fn = "data/minmax_";
  string mask_fn = "data/mask_";
  string center_fn = "data/center.jp2";
  string partition_fn = "data/partition";
  string partition_labels_fn = "data/partition_labels";
  string decoded_fn = "data/decoded_";
  ...
  if(decode) {
    ...
    //A_icjc->writeImage("center.txt");
    compress_with_jp2(A_icjc, string(center_fn));

    // segment center view with mean shift
    Image<int>* BqFs = segment(A_icjc);
    Image<int>* BqF = order_segmentation_by_depth(BqFs, views);
    delete BqFs;

    bool optimize_regions = parser.cmdOptionExists("-o");
    if( optimize_regions ) {
      vector< Image< uint8_t >* >* A_regressors = new vector< Image< uint8_t >* >();
      int n = 20;
      ...
      for(int i = 0; i < merged_regions.size(); ++i ) {
        cout << i << ": ";
        for(int j = 0; j < merged_regions.at(i).size(); ++j ) {
          cout << merged_regions.at(i).at(j) << " ";
        }
        cout << endl;
      }
      merge_center(BqF, merged_regions);
      BqF->writeImage("merged.txt");
    }
